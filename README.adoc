...
The pulsarj library is a Java implementation of the Pulsar protocol, which allows it to maintain a wallet and send/receive transactions without needing a local copy of Pulsar Core. It comes with full documentation and some example apps showing how to use it.
...
The *pulsarj* `wallettool` subproject includes a command-line Wallet Tool (`wallet-tool`) that can be used to create and manage *pulsarj*-based wallets (both the HD keychain and SPV blockchain state.) Using `wallet-tool` on Pulsar coin's test net is a great way to learn about Pulsar coin and *pulsarj*.

To build an executable shell script that runs the command-line Wallet Tool, use:
```
gradle pulsarj-wallettool:installDist
```
...
To create a test net wallet file in `~/pulsarj/pulsarj-test.wallet`, you would use:
```
mkdir ~/pulsarj
```
```
./wallettool/build/install/wallet-tool/bin/wallet-tool --net=TESTNET --wallet=$HOME/pulsarj/pulsarj-test.wallet create
```

To sync the newly created wallet in `~/pulsarj/pulsarj-test.wallet` with the test net, you would use:
```
./wallettool/build/install/wallet-tool/bin/wallet-tool --net=TESTNET --wallet=$HOME/pulsarj/pulsarj-test.wallet sync
```

To dump the state of the wallet in `~/pulsarj/pulsarj-test.wallet` with the test net, you would use:
```
./wallettool/build/install/wallet-tool/bin/wallet-tool --net=TESTNET --wallet=$HOME/pulsarj/pulsarj-test.wallet dump
```
